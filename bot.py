    ...
    if texto == '/menu':
        ...
        if (from_id ==684842099):####coloque seu-id
            if len_msg > 1:
                ...
                try:bot.sendMessage(-1001224597403, texto, parse_mode="Markdown")
                except:
                    try:bot.sendMessage(-1001224597403, texto)
        ...
        markup = ReplyKeyboardMarkup(keyboard=[['OCULTAR', KeyboardButton(text='✌ Meu Canal ')],[dict(text='INFO')],])
        bot.sendMessage(chat_id, '[{}](telegram.me/{})'.format('Robo taylor', 'odouspam',), parse_mode=M, disable_web_page_preview=True, reply_markup=markup, reply_to_message_id=msg_id)
    elif texto == 'INFO':
        a= bot.getMe()
        n = a['first_name']
        u = a['username']
...
username: {}
*_________*
[Criador](https://telegram.me/Odouspam) '''.format(n, u), parse_mode=M)
    elif texto == '/chat_id':
        ...
        if (from_id == 684842099) or (from_id == 1000660479):
            if len_msg > 1:
                ...
                arq.write ('• '+texto+'\n')
                bot.sendMessage(chat_id, '_Lista Atualizada!_', parse_mode="Markdown")     
                arq.close()          
        else:
...
fale com_ @paulo6'''.format(texto), parse_mode="Markdown")
        ...
        time.sleep(2)
        markup = InlineKeyboardMarkup(inline_keyboard=[[dict(text='🔧suporte', url='https://telegram.me/Odouspam')],[InlineKeyboardButton(text='📃status', callback_data='n')]+[InlineKeyboardButton(text='⚙Comandos', callback_data='edit')]+[InlineKeyboardButton(text='ℹinfo', callback_data='info')]])
        ...
        markup = InlineKeyboardMarkup(inline_keyboard=[[dict(text='📡✌ Meu Grupo ', url='https://t.me/joinchat/KNHccxT3lnygNpdyUvfHZw')],[InlineKeyboardButton(text='👤Adms', callback_data='adm')]+[InlineKeyboardButton(text='👥grupo', callback_data='edit')],[InlineKeyboardButton(text='🛠 remover usuário', callback_data='ban')]+ [dict(text='Link🌐', callback_data='link')], [InlineKeyboardButton(text='extras 🕹', callback_data='extra')]+[InlineKeyboardButton(text='📝Sobre o grupo', callback_data='sobre')],[InlineKeyboardButton(text='⚙DEVS', callback_data='dev')]])
        global message_with_inline_keyboard
...
Deixa eu te fala, você pode entrar em contato com a minha equipe, a (Equipe Sk)[t.me/ListaSk], lá eles oferecem suporte rsrs.""".format(msg['from']['first_name']), reply_markup=markup, parse_mode="Markdown")
...
def on_callback_query(msg):
    ...
    if data == 'n':
        ...
        if message_with_inline_keyboard:
            ...
            markup = InlineKeyboardMarkup(inline_keyboard=[[dict(text='🔧suporte', url='https://telegram.me/odouspam')],[InlineKeyboardButton(text='📃status', callback_data='n')]+[InlineKeyboardButton(text='⚙', callback_data=' ')]+[InlineKeyboardButton(text='ℹinfo', callback_data='info')]])
...
EXEMPLO: */lista*''', parse_mode="Markdown", reply_markup=markup)
        ...
        if message_with_inline_keyboard:
            msg_idf = telepot.message_identifier(message_with_inline_keyboard)
            markup = InlineKeyboardMarkup(inline_keyboard=[[dict(text='🔧suporte', url='https://telegram.me/Odouspam')],[InlineKeyboardButton(text='📃status', callback_data='n')]+[InlineKeyboardButton(text='⚙Comandos', callback_data='edit')]+[InlineKeyboardButton(text='ℹ', callback_data=' ')]])
            bot.editMessageText(msg_idf, '''
🤖*Informações do bot:*
➖➖➖➖➖➖➖➖➖
*bot*: `{}`
...
*username:*` {}`
➖➖➖➖
[Criador](https://telegram.me/Odouspam) '''.format(n, u), disable_web_page_preview=True, parse_mode="Markdown", reply_markup=markup)
...
############# END ###############
bot = telepot.Bot("965202811:AAH1keloPWoiV793bJ4cJV22NKKK3gUFIsY")####token  bot
